...
- WebDAV：**一个统一入口 `/dav/` 就是全部挂载**（不用一个个加）；单个挂载也能单独挂 `/dav/<挂载名>/`；独立应用密码，可挂载进 Windows 资源管理器 / 手机（内网手机访问走这条）
  ...
  - **本机有多个网卡 / 多个组网时**（VMware、Hyper-V、ZeroTier、Tailscale 之类都会各多出一个地址）：设置页 WebDAV 区会把**所有可用地址**逐条列出来，带网卡名、虚拟网卡标记与「当前」标记，每条一个独立复制按钮 —— 复制哪条由你决定，不用再靠切网卡试；管理台复制挂载路径、分享链接也是弹出地址选择器
...
- 端口：默认 `18322`，`CP_PORT` 修改；对外可达地址用 `CP_PUBLIC_URL`；部署在反向代理之后时用 `CP_TRUSTED_PROXIES` 声明代理网段。**多网卡 / 多组网的机器**：启动日志会把本机所有可访问地址逐条打印出来（带网卡名），设置页 WebDAV 区也会全部列出供你选择复制，不再只给一个猜出来的地址
