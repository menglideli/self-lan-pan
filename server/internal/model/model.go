...
type User struct {
	...
	// 个人配额覆盖（MB）：<0 = 跟随权限档案；0 = 不限量；>0 = 该用户专属上限
...
// Perms 权限档案。
//
// 2026-09 单用户私有化改造：**用户组概念已彻底删除**——不再有用户组表与用户组管理接口，
// 也不再按 User.GroupID 下发权限。系统只服务一个管理员账号，权限在代码里写死为"全开"，
// 唯一来源就是下面的 AdminPerms()。本结构保留为纯值对象（不入库），仅为承接各处
// "读权限标志"的调用点。
type Perms struct {
	Name                 string `json:"name"`
	QuotaMB              int64  `json:"quotaMB"` // -1 = 不限量
	...
	RecycleRetentionDays int    `json:"recycleRetentionDays"` // 回收站保留天数；0 = 永久保留（不自动清理）
	KeepVersions         int    `json:"keepVersions"`         // 每个文件保留的历史版本数，-1 = 不限
	VersionRetentionDays int    `json:"versionRetentionDays"` // 版本保留天数，0 = 永久
	AllowedPolicyIDs     string `json:"allowedPolicyIDs"`
	AppPerms             string `json:"appPerms"`
	Remark               string `json:"remark"`
}

// AdminPerms 唯一权限档案：管理员全开，全部由代码写死。
// 唯一用户就是管理员，所以它同时是"默认档"和"最高档"。
// RecycleRetentionDays 取 0（永久保留）：不做任何"按时间自动删除用户文件"的行为。
func AdminPerms() *Perms {
	return &Perms{
		Name:                 "管理员",
		QuotaMB:              -1, // 不限量：自己的内网私有盘
		AllowShare:           true,
		AllowWebdav:          true,
		AllowArchive:         true,
		AllowOffline:         true,
		...
		ReadOnly:             false,
		DownloadSpeedKB:      0, // 不限速
		RecycleRetentionDays: 0, // 永久保留
		KeepVersions:         10,
		VersionRetentionDays: 0,
		AllowedPolicyIDs:     "", // 空 = 所有挂载点可用
		AppPerms:             "", // 空 = 除全局开关外全部可用
		Remark:               "单用户私有部署：权限写死在代码里，管理员全开",
	}
}

func (g *Perms) PolicyIDList() []uint {
	var ids []uint
	_ = json.Unmarshal([]byte(g.AllowedPolicyIDs), &ids)
	return ids
}

func (g *Perms) CanUsePolicy(pid uint) bool {
	ids := g.PolicyIDList()
	if len(ids) == 0 {
		return true
	}
...
// seed 首次部署初始化：只做"建管理员账号"与站点默认设置两项。
//
// 单用户私有化改造（2026-09）后不再有：默认用户组、访客组、游客账号、站内共享。
// 权限一律来自 model.AdminPerms()（代码写死），不再入库。
func seed() {
	var uc int64
	...
	if uc == 0 {
		...
		DB.Create(&User{Username: "admin", PasswordHash: hashPwd(adminPwd), Nickname: "管理员", Role: "admin", QuotaMB: -1})
		log.Printf("[CloudPan] 首次部署：初始管理员密码为 %s（仅本次日志输出，登录请立即修改）", adminPwd)
	}
	// 历史遗留清理：系统托管的 guest 游客账号（单用户私有部署不再提供游客登录）。
	// 只删账号行；若该账号曾往盘里写过文件，文件本身不动。
	if res := DB.Where("username = ?", "guest").Delete(&User{}); res.Error == nil && res.RowsAffected > 0 {
		log.Printf("[CloudPan] 已移除历史游客账号：单用户私有部署不再提供游客登录")
	}
	var sc int64
